#!/usr/bin/env python3
"""
AppleMAC-LED macOS agent.

The agent observes selected macOS activity and drives an ESP32-S3 WS2812B status
light over USB Serial. This public build contains only the macOS agent and LED/status-light functionality.

Commands:
  status, reboot, snake-on/off, copy-on/off, chatgpt-on/off,
  appstore-on/off, trash-flash, system-blue, ports,
  usb-pause/resume/reconnect, daemon
"""
...
def detach_supervised_daemon() -> None:
    ...
    pid_file = Path(os.environ.get(
        "APPLEMACLED_DAEMON_PID_FILE",
        str(Path.home() / "Library" / "Application Support" / "AppleMAC-LED" / "daemon.pid"),
    ))
...
try:
    ...
    print("Run this through applemacled.command", file=sys.stderr)
    raise SystemExit(2)
...
try:
    ...
    print("The macOS AppKit module is not installed.", file=sys.stderr)
    print("Run this through applemacled.command", file=sys.stderr)
    raise SystemExit(2)


def configure_background_appkit() -> None:
    """Prevents the background Python process from appearing in the Dock.

    On the first NSEvent send, macOS can otherwise register the regular Python
    process as a GUI application. Because the process has no window loop, the
    Dock may show a Python icon and mark it as unresponsive.
    """
    try:
        ...
        print(
            "Failed to enable AppKit background mode: %s" % error,
            file=sys.stderr,
        )
...
def chatgpt_ax_scan_hit_limit(diagnostic: str) -> bool:
    if "stop=not-found" not in diagnostic:
        return False
    match = re.search(r"\belements=(\d+)\b", diagnostic)
...
class CodexSessionActivityMonitor:
    ...
    def _last_lifecycle_marker_in_file(
        self,
        path: Path,
        size: int,
    ) -> Optional[bool]:
    ...
    def poll(self, now: float, present: bool) -> Tuple[bool, bool, bool, int]:
        ...
        if now >= self.next_scan_at:
            ...
            for state in self.files.values():
                if (
                    state.get("active")
                    and now_wall - float(state.get("updated_at", 0.0))
...
class NativeUIStateMonitor:
    """Reads state collected by the native application launcher."""
    ...
    def poll(self) -> NativeUIState:
        try:
            ...
            if mtime_ns != self.last_mtime_ns:
                ...
                state = NativeUIState(
                    ax_trusted=bool(payload.get("axTrusted", False)),
                    front_bundle=str(payload.get("frontBundle", "") or ""),
                    front_name=str(payload.get("frontName", "") or ""),
                    bluetooth_event_counter=int(payload.get("bluetoothEventCounter", 0) or 0),
                    bluetooth_device_name=str(payload.get("bluetoothDeviceName", "") or ""),
                    finder_copy_active=bool(payload.get("finderCopyActive", False)),
                    finder_foreground=bool(payload.get("finderForeground", False)),
                    finder_diagnostic=str(payload.get("finderDiagnostic", "") or ""),
                    trash_event_counter=int(payload.get("trashEventCounter", 0) or 0),
                    chatgpt_tab_active=bool(payload.get("safariChatGPTTabActive", False)),
                    chatgpt_busy=bool(payload.get("safariChatGPTBusy", False)),
                    safari_foreground=bool(payload.get("safariForeground", False)),
                    safari_diagnostic=str(payload.get("safariDiagnostic", "") or ""),
                    safari_download_active=bool(payload.get("safariDownloadActive", False)),
                    safari_download_count=int(payload.get("safariDownloadCount", 0) or 0),
                    safari_download_diagnostic=str(payload.get("safariDownloadDiagnostic", "") or ""),
                    appstore_download_active=bool(payload.get("appStoreDownloadActive", False)),
                    appstore_diagnostic=str(payload.get("appStoreDiagnostic", "") or ""),
                    openai_app_active=bool(payload.get("openAIAppActive", False)),
                    openai_app_busy=bool(payload.get("openAIAppBusy", False)),
                    openai_app_foreground=bool(payload.get("openAIAppForeground", False)),
                    openai_app_name=str(payload.get("openAIAppName", "") or ""),
                    openai_app_diagnostic=str(payload.get("openAIAppDiagnostic", "") or ""),
                    error="" if age <= self.STALE_SECONDS else "native UI monitor is stale",
                )
                ...
                if updated_age > self.STALE_SECONDS:
                    self.last_good.error = "native UI monitor is not updating"
            return self.last_good
        except FileNotFoundError:
            self.last_good.error = "native monitor state file has not been created yet"
            return self.last_good
        except Exception as error:
            self.last_good.error = "failed to read native monitor state: %s" % error
            return self.last_good
...
class NativeAudioStateMonitor:
    """Reads fast system-audio levels from the native ScreenCaptureKit monitor."""
        ...
        try:
            ...
            if mtime_ns != self.last_mtime_ns:
                ...
                state = NativeAudioState(
                    active=bool(payload.get("active", False)) and healthy,
                    level=clamp_unit_float(payload.get("level", 0.0)) if healthy else 0.0,
                    peak=clamp_unit_float(payload.get("peak", 0.0)) if healthy else 0.0,
                    beat=bool(payload.get("beat", False)) and healthy,
                    updated_at=updated_at,
                    fresh=fresh,
                    diagnostic=diagnostic,
                    error=(diagnostic if monitor_failed else ("" if fresh else "audio monitor is stale")),
                )
                self.last_good = state
                self.last_mtime_ns = mtime_ns
            else:
                ...
                if updated_age > self.STALE_SECONDS:
                    ...
                    self.last_good.error = "audio monitor is not updating"
            return self.last_good
        except FileNotFoundError:
            self.last_good.active = False
            self.last_good.level = 0.0
            ...
            self.last_good.beat = False
            self.last_good.fresh = False
            self.last_good.error = "audio state file has not been created yet"
            return self.last_good
        except Exception as error:
            self.last_good.active = False
            self.last_good.level = 0.0
            ...
            self.last_good.beat = False
            self.last_good.fresh = False
            self.last_good.error = "failed to read audio monitor state: %s" % error
            return self.last_good
...
async def audio_serial_loop(provisioner) -> None:
    ...
    start_confirm_seconds = 0.90
    active_hold_seconds = 1.15

    while True:
        now = time.monotonic()
        if last_audio_iteration_at > 0.0:
            max_audio_loop_gap = max(
                max_audio_loop_gap,
                now - last_audio_iteration_at,
            )
        last_audio_iteration_at = now
        state = monitor.poll()
        sample_changed = bool(
            state.updated_at > 0.0
            and state.updated_at != last_native_updated_at
        )
        beat = False
        ...
        if sample_changed:
            ...
            latest_raw_active = bool(
                not state.error
                and state.fresh
                and state.active
                and (state.level >= 0.015 or state.peak >= 0.04)
            )
            ...
            if latest_raw_active:
                ...
                if baseline_level > 0.0 and baseline_peak > 0.0:
                    ...
                    derived_beat = bool(
                        now - last_beat_at >= 0.18
                        and state.peak >= 0.14
                        and (
                            (peak_rise >= 0.055 and peak_ratio >= 1.05)
                            or peak_ratio >= 1.30
                            or (
                                level_rise >= 0.045
                                and level_ratio >= 1.06
                            )
                        )
                    )
                else:
                ...
                # A detected transient must not immediately raise the baseline,
                # otherwise the next real beat becomes invisible.
                baseline_alpha = 0.015 if beat else 0.060
        ...
        active = bool(
            now <= active_hold_until
            and (smooth_level >= 0.010 or smooth_peak >= 0.020)
        )

        if state.diagnostic and state.diagnostic != last_diagnostic:
            
            
            if state.diagnostic not in {
                "ScreenCaptureKit audio active",
                "ScreenCaptureKit audio idle",
            }:
        ...
        if state.error:
            ...
            print("Audio: system audio monitor is ready.")
            last_error = ""

        if active or was_active:
            sequence = (sequence + 1) & 0xFFFFFFFF
            output_level = audio_byte(
                smooth_level if active else 0.0,
                gain=0.95,
                gamma=0.85,
            )
            output_peak = audio_byte(
                smooth_peak if active else 0.0,
                gain=1.35,
                gamma=0.75,
            )
            try:
                send_started_at = time.monotonic()
                provisioner.send_audio_realtime(
                    sequence,
                    output_level,
                    output_peak,
                    bool(beat if raw_active and active else False),
                    active,
                )
                max_send_seconds = max(
                    max_send_seconds,
                    time.monotonic() - send_started_at,
                )
            except Exception as error:
                if now - last_send_error_at >= 10.0 and not provisioner.suspended:
                    print("Audio USB: failed to send level to ESP: %s" % error)
                    last_send_error_at = now

            if active and now - last_stream_diagnostic_at >= 20.0:
                print(
                    "Audio stream: level=%d, peak=%d, beats=%d, "
                    "native-gap-max=%.3fs, loop-gap-max=%.3fs, "
                    "usb-send-max=%.3fs."
                    % (
                        output_level,
                        output_peak,
        ...
        if active and not was_active:
            ...
            print("Audio: system audio became quiet; disabling the music overlay.")
        was_active = active
...
def audio_serial_thread_main(provisioner) -> None:
    ...
    try:
        ...
        print("Audio thread: music stream stopped unexpectedly: %s" % error)


class WiFiConnectionEventMonitor:
    """Emits an event only when the Mac newly connects to Wi-Fi or changes SSID."""
...
class AppStoreProcessMonitor:
    ...
    The native UI/I/O monitor is primary. This fallback intentionally does not
    watch appstoreagent/appstored/commerce/installd because they often become
    active merely when App Store is opened and do not imply a download/update.
    """
    ...
    def poll(self, now: float) -> Tuple[bool, str]:
        ...
        if details:
            self.last_diag = "download-process=%s, hold=%s" % (
                ",".join(details),
                "yes" if now < self.active_until else "no",
            )
        else:
            self.last_diag = "App Store download processes are not active"
        return now < self.active_until, self.last_diag


class ArduinoIdeActivityMonitor:
    """Detects an actual compile or upload from Arduino IDE."""
    ...
    def __init__(self) -> None:
        ...
        self.last_diag = "Arduino IDE is waiting for a compile or upload"
    ...
    def classify_command(cls, command: str) -> str:
        ...
        if is_esptool and (
            "write_flash" in normalized or
            "write-flash" in normalized
        ):
            return "upload"
        if is_esptool and (
            "merge_bin" in normalized or
            "merge-bin" in normalized
        ):
            return "compile"
        ...
        if "/arduino15/packages/" in normalized:
            if any(
                ...
                for pattern in cls.PACKAGE_COMPILE_PATTERNS
            ):
                return "compile"
    ...
    def upload_targets_main_usb(cls, command: str, main_usb_port: str) -> bool:
        """True only for an upload that explicitly uses the main ESP USB connection."""
        normalized = command.casefold()
        ...
        if not main_aliases:
            return False
        command_ports = set(re.findall(
            r"/dev/(?:cu|tty)\.[a-zA-Z0-9._-]+",
            command,
        ))
    ...
    def poll(
        ...
        if now < self.next_poll_at:
            ...
            return (
                ...
                bool(
                    active_mode == "upload"
                    and self.last_upload_uses_main_usb
                ),
            )
        self.next_poll_at = now + self.POLL_SECONDS
        ...
        classified_commands = [
            (self.classify_command(line), line)
            for line in result.stdout.splitlines()
            if line.strip()
        ]
        ...
        if mode:
            self.last_mode = mode
            self.active_until = now + self.HOLD_SECONDS
            self.last_upload_uses_main_usb = bool(
                mode == "upload"
                and any(
                    command_mode == "upload"
                    and self.upload_targets_main_usb(command, main_usb_port)
                    for command_mode, command in classified_commands
                )
            )
        ...
        if active_mode == "upload":
            if self.last_upload_uses_main_usb:
                self.last_diag = (
                    "Arduino IDE is uploading to the main ESP over USB; "
                    "the port is being released temporarily"
                )
            else:
                self.last_diag = (
                    "Arduino IDE is uploading to another board or over OTA; "
                    "the main ESP USB connection remains active"
                )
        elif active_mode == "compile":
            self.last_diag = "Arduino IDE is compiling a sketch"
        else:
            self.last_diag = "Arduino IDE is not compiling/uploading"
        return (
            active,
            active_mode,
            self.last_diag,
...
def find_wifi_interface() -> str:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            (result.stderr or "").strip() or "networksetup failed"
        )
    ...
    raise RuntimeError("Mac Wi-Fi interface was not found")
...
def get_current_ssid_fallback() -> str:
    ...
    if result.returncode == 0 and output:
        ...
        if (
            "not associated" not in lowered
            and "не подключ" not in lowered
            and "<redacted>" not in lowered
        ):
            match = re.search(
    ...
    match = re.search(r"^\s*SSID\s*:\s*(.+?)\s*$", summary.stdout, re.MULTILINE)
    if match and match.group(1).strip():
        return match.group(1).strip()

    raise RuntimeError("Failed to determine the current Wi-Fi network")


def parse_decision(status: str) -> str:
    match = re.search(r"(?:^|\|)DECISION:(NONE|OK|NC)(?:\||$)", status)
    return match.group(1) if match else "NONE"


def get_interface_io_bytes(interface: str) -> Tuple[int, int]:
    """Returns (received bytes, sent bytes) for an interface."""
    ...
    if result.returncode == 0:
        ...
        for line in lines:
            ...
            if (
                ibytes_index is not None
                and obytes_index is not None
                and columns
                and columns[0] == interface
                and len(columns) > max(ibytes_index, obytes_index)
            ):
                try:
                    values.append((
                        int(columns[ibytes_index]),
                        int(columns[obytes_index]),
                    ))
                except ValueError:
                    pass
    ...
    input_match = re.search(
        r"input packets\s+\d+\s+bytes\s+(\d+)",
        fallback.stdout,
    )
    output_match = re.search(
        r"output packets\s+\d+\s+bytes\s+(\d+)",
        fallback.stdout,
    )
    if input_match and output_match:
        return int(input_match.group(1)), int(output_match.group(1))

    raise RuntimeError("Failed to read traffic counters for %s" % interface)



class AirDropTrafficMonitor:
    """Quickly detects both incoming and outgoing AirDrop transfers."""

    def __init__(self, threshold_kbps: float = 24.0, sample_seconds: float = 0.10) -> None:
    ...
    def poll(self) -> Tuple[bool, float, str]:
        ...
        if totals is not None and self.previous is not None and self.previous_time is not None:
            ...
            if tx_rate > rx_rate * 1.20:
                self.direction = "AirDrop upload"
            elif rx_rate > tx_rate * 1.20:
                self.direction = "AirDrop download"
            else:
                self.direction = "AirDrop"

            
            if self.last_rate >= self.threshold_bytes or (
                connection and self.last_rate >= 4.0 * 1024.0
            ):
...
class SerialProvisioner:
    """Self-recovering USB Serial transport to the ESP32-S3."""
    ...
    def __init__(
        ...
        # PySerial supports one reader and one writer concurrently. This lock
        # prevents byte interleaving between ordinary commands and audio frames
        # without blocking audio while another thread waits for an ESP reply.
    ...
    def _candidate_ports(self) -> List[str]:
        ...
        for item in list_ports.comports():
            ...
            if device.startswith("/dev/cu.usbmodem") and (
                "esp" in description or "espressif" in manufacturer
            ):
    ...
    def list_ports_text(self) -> str:
        ...
        for item in list_ports.comports():
            rows.append(
                "%s — %s%s" % (
                    getattr(item, "device", "?"),
                    getattr(item, "description", "unknown device"),
                    " [VID:PID=%04X:%04X]" % (item.vid, item.pid)
                    if getattr(item, "vid", None) is not None and getattr(item, "pid", None) is not None
                    else "",
                )
            )
    ...
    def _read_response_sync(self, prefixes: Sequence[str], timeout: float) -> Optional[str]:
        ...
        while time.monotonic() < deadline:
            try:
                ...
                self._close_sync()
                raise SerialAgentError("USB Serial read interrupted: %s" % error)
            if not raw:
                continue
    ...
    def _write_line_sync(self, command: str, *, flush_output: bool = False) -> None:
        with self._write_lock:
            if self._serial is None:
                raise SerialAgentError("USB Serial not connected")
            ...
            try:
                ...
                self._close_sync()
                raise SerialAgentError("USB Serial write interrupted: %s" % error)
    ...
    def _probe_port_sync(self, port_name: str):
        ...
        try:
            ...
            time.sleep(1.0 if native_usb_cdc else 0.20)
    ...
    def _ensure_serial_sync(self):
        ...
        if not candidates:
            raise SerialAgentError(
                "ESP32-S3 USB Serial was not found. Check the cable and USB CDC On Boot."
            )
        ...
        for port_name in candidates:
            try:
                ...
                print("Persistent USB Serial connection to ESP32 established: %s." % port_name)
                return handle
            except Exception as error:
                errors.append("%s (%s)" % (port_name, error))

        raise SerialAgentError(
            "ESP32-S3 is not responding over USB Serial. Checked: %s" % "; ".join(errors)
        )
    ...
    def _request_sync(
        ...
        if result is None:
            self._close_sync()
            raise SerialAgentError("ESP32 did not respond to %s" % command)
        if result.startswith("ERROR:"):
            raise SerialAgentError("ESP32 rejected %s: %s" % (command, result))
        return result
    ...
    async def set_suspended(self, suspended: bool) -> None:
        ...
        if suspended:
            ...
            print("USB Serial released for Arduino firmware upload.")
        else:
            print("Arduino upload finished; USB Serial is available to the agent again.")
            self._disconnect_generation += 1

    async def force_reconnect(self) -> None:
        """Close even a formally open descriptor and rediscover the ESP."""
        async with self._lock:
            await asyncio.to_thread(self._close_sync, disconnected=False)
        self._disconnect_generation += 1
    ...
    async def _command(self, command: str, expected: str, timeout: float = 4.0) -> int:
        def operation():
            ...
            if result != expected:
                raise SerialAgentError("ESP32 did not accept %s: %s" % (command, result))
            return 0
    ...
    async def get_status_persistent(self) -> Tuple[str, Optional[ipaddress.IPv4Address]]:
        def operation():
            return self._request_sync("STATUS", ["USB:", "ERROR:"], 6.0)
        status = await self._run(operation)
        return status, None

    async def get_status(self) -> Tuple[str, Optional[ipaddress.IPv4Address]]:
    ...
    def send_audio_realtime(
        ...
        command = "AUDIO:%u:%u:%u:%u:%u" % (
            sequence & 0xFFFFFFFF,
            max(0, min(255, int(level))),
            max(0, min(255, int(peak))),
            1 if beat else 0,
            1 if active else 0,
        )

        # Never start port discovery from the real-time audio path. The health
        # loop owns reconnects; audio only uses an already-open CDC descriptor.
        if self._suspended:
            raise SerialAgentError("USB Serial is temporarily released for Arduino flashing")
        if self._serial is None or not self._serial.is_open:
            raise SerialAgentError("USB Serial not connected")
        if self._port_name and not os.path.exists(self._port_name):
            raise SerialAgentError("USB Serial port disappeared")

        # The 40 Hz loop already runs in its own OS thread. A direct write here
        # avoids executor scheduling jitter while RLock keeps command bytes
        # atomic against other USB command writes.
        self._write_line_sync(command, flush_output=False)


PersistentSerialProvisioner = SerialProvisioner
...
async def send_command_to_running_agent(command: str, **payload) -> Optional[dict]:
    """Sends a manual command to the already running agent.

    The agent keeps the USB Serial port open continuously. IPC lets manual
    commands reuse the existing port without conflicting with the background process.
    """
    if not IPC_SOCKET.exists():
        return None
    try:
        reader, writer = await asyncio.wait_for(
            asyncio.open_unix_connection(str(IPC_SOCKET)),
            timeout=1.0,
        )
    ...
    try:
        ...
        try:
            ...
            return {
                "ok": False,
                "message": "The agent did not finish the command within 15 seconds.",
            }
        if not raw:
            raise RuntimeError("the agent closed IPC without a response")
        response = json.loads(raw.decode("utf-8", errors="replace"))
        if not isinstance(response, dict):
            raise RuntimeError("invalid IPC response from the agent")
        return response
    finally:
        writer.close()
        try:
...
async def daemon_loop(
    provisioner: SerialProvisioner,
    *,
    health_interval: float,
    copy_poll_interval: float,
) -> int:
    print("AppleMAC-LED daemon 36.14 Lighting Only started.")
    print("ESP USB connectivity is checked immediately at startup and then every %.0f seconds." % health_interval)
    print("Finder, Trash, Safari, App Store, Arduino IDE, ChatGPT/Codex, Bluetooth, Wi-Fi, and system audio are monitored by the agent.")
    print("Desktop/Documents/Downloads permissions are no longer required for copy detection.")
    print("Safari and App Store downloads are treated as long-running system modes; ChatGPT/Codex remains tracked in the background, while AirDrop, Trash, and Bluetooth/Wi-Fi have system priority.")
    ...
    async def handle_ipc(reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        try:
            ...
            if command == "chatgpt-on":
                ...
                chatgpt_active = True
                response = {"ok": True, "message": "Orange snake enabled for 20 seconds through the running agent."}
            elif command == "chatgpt-off":
                manual_chatgpt_until = 0.0
                await provisioner.set_chatgpt_snake(False)
                chatgpt_active = False
                response = {"ok": True, "message": "Orange snake disabled."}
            elif command == "copy-on":
                manual_copy_until = now + 20.0
                await provisioner.set_copy_snake(True)
                copy_active = True
                response = {"ok": True, "message": "Light-green snake enabled for 20 seconds through the running agent."}
            elif command == "copy-off":
                manual_copy_until = 0.0
                await provisioner.set_copy_snake(False)
                copy_active = False
                response = {"ok": True, "message": "Light-green snake disabled."}
            elif command == "appstore-on":
                manual_appstore_until = now + 20.0
                await provisioner.set_appstore_snake(True)
                appstore_active = True
                response = {"ok": True, "message": "App Store aurora snake enabled for 20 seconds."}
            elif command == "appstore-off":
                manual_appstore_until = 0.0
                await provisioner.set_appstore_snake(False)
                appstore_active = False
                response = {"ok": True, "message": "App Store aurora snake disabled."}
            elif command == "trash-flash":
                await provisioner.flash_trash()
                response = {"ok": True, "message": "Three red flashes sent to ESP32."}
            elif command == "system-blue":
                await provisioner.system_blue()
                response = {"ok": True, "message": "Blue system snake pass sent to ESP32."}
            elif command == "snake-on":
                await provisioner.set_snake(True)
                response = {"ok": True, "message": "Service snake enabled."}
            elif command == "snake-off":
                await provisioner.set_snake(False)
                response = {"ok": True, "message": "Service snake disabled."}
            elif command == "usb-pause":
                manual_usb_paused = True
                await provisioner.set_suspended(True)
                response = {"ok": True, "message": "USB Serial port released. You can flash the ESP or open Serial Monitor."}
                ...
                response = {
                    "ok": True,
                    "message": "USB Serial descriptor reset; the agent will rediscover the native ESP32-S3 port.",
                }
            elif command == "status":
                status, _unused = await provisioner.get_status_persistent()
                response = {
                    "ok": True,
                    "message": "Final status: %s\nUSB port: %s" % (
                        status,
                        provisioner.port_name,
                    ),
                }
            elif command == "reboot":
                await provisioner.reboot_persistent()
                response = {"ok": True, "message": "ESP32 is rebooting."}
        except Exception as error:
    ...
    try:
        IPC_SOCKET.chmod(0o600)
    except OSError:
        pass
    print("Local agent command channel ready: %s" % IPC_SOCKET)
    audio_thread = threading.Thread(
        target=audio_serial_thread_main,
        args=(provisioner,),
        name="AppleMACLED-Audio-Serial",
        daemon=True,
    )
    audio_thread.start()
    print("Audio: dedicated 40 Hz stream started separately from the system monitors.")
    ...
    while True:
        ...
        (
            ...
            arduino_mode,
            arduino_diag,
            arduino_upload_uses_main_usb,
        ) = await asyncio.to_thread(
            arduino_activity_monitor.poll,
            loop_started,
            main_usb_port_hint,
        )
        arduino_upload_in_progress = bool(
            arduino_active and arduino_mode == "upload"
        )
        arduino_main_usb_upload_in_progress = bool(
            arduino_upload_in_progress and arduino_upload_uses_main_usb
        )
        usb_transport_paused = bool(
            manual_usb_paused or arduino_main_usb_upload_in_progress
        )
        ...
        if current_disconnect_generation != last_usb_disconnect_generation:
            ...
            startup_wait_sent = False
            print(
                "USB Serial: connection lost; the agent will automatically rediscover the port."
            )
        ...
        if usb_transport_paused and loop_started >= next_health_at:
            ...
            try:
                if startup_sync_pending and not startup_wait_sent:
                    ...
                    print("[%s] ESP switched to Mac-waiting mode over USB." % (
                ...
                if startup_sync_pending:
                    retry_delay = 2.0
                    startup_wait_sent = False
                print("[%s] ESP USB check failed: %s" % (
                    time.strftime("%Y-%m-%d %H:%M:%S"), error,
                ))
        ...
        if ui_state.error:
            ...
            if (
                not ui_parent_restart_sent
                and ui_error_started_at > 0.0
                and now - ui_error_started_at >= ui_parent_restart_seconds
            ):
                ui_parent_restart_sent = True
                parent_pid = os.getppid()
                print(
                    "Native UI monitor has stalled for %.0f seconds; restarting AppleMACLED Agent."
                    % (now - ui_error_started_at)
                )
                if parent_pid > 1:
                    try:
                        os.kill(parent_pid, signal.SIGKILL)
                    except OSError as error:
                        print("Failed to stop the parent monitor: %s" % error)
                os._exit(75)
        elif last_ui_error:
            print("Native UI monitor is working again.")
            ui_error_started_at = 0.0
            ui_parent_restart_sent = False
        ...
        if last_bluetooth_event_counter is None:
            ...
            print(
                "Bluetooth: %s connected; queueing blue system indication."
                % device_name
            )
        ...
        if wifi_ssid:
            ...
            print(
                "Wi-Fi: Mac connected to %s; queueing blue system indication."
                % wifi_ssid
            )
        ...
        codex_session_busy, codex_session_started, codex_session_stopped, codex_active_count = (
            await asyncio.to_thread(
                codex_session_monitor.poll,
                now=now,
                present=bool(ui_state.openai_app_active),
            )
        )
        if codex_session_started:
            print(
                "Codex sessions: active task detected; "
                "enabling indication regardless of the selected chat."
            )
        elif codex_session_stopped:
            print(
                "Codex sessions: all active tasks finished; "
                "disabling the application indication immediately."
            )
        safari_chatgpt_scan_incomplete = chatgpt_tab_present and chatgpt_ax_scan_hit_limit(
            ui_state.safari_diagnostic
        )
        ...
        if chatgpt_tab_present and not chatgpt_tab_was_active:
            print("Safari ChatGPT: selected tab detected; the task will continue to be tracked in the background.")
        elif not chatgpt_tab_present and chatgpt_tab_was_active:
            print("Safari ChatGPT: selected tab is no longer detected.")
        ...
        if openai_app_present and not openai_app_was_active:
            ...
            print("%s: window detected; ChatGPT/Codex activity will continue to be tracked in the background." % app_name)
        elif not openai_app_present and openai_app_was_active:
            print("ChatGPT/Codex application is no longer detected.")
        ...
        if finder_copy_active and not finder_copy_was_active:
            print("Finder: copy confirmed; keeping indication active for at least 2 seconds.")
        elif not finder_copy_active and finder_copy_was_active:
            print("Finder: copy finished after a stable OFF state.")
        finder_copy_was_active = finder_copy_active

        airdrop_active, airdrop_rate, airdrop_direction = await asyncio.to_thread(
            airdrop_monitor.poll
        )
        ...
        if safari_download_count != last_safari_download_count:
            if safari_download_count > 0:
                print("Safari: active downloads: %d; enabling the system light-green snake." % safari_download_count)
            elif last_safari_download_count > 0:
                print("Safari: last download finished; restoring the previous indication.")
            last_safari_download_count = safari_download_count

        process_appstore_active, process_appstore_diag = await asyncio.to_thread(
            appstore_process_monitor.poll,
            now,
        )
        appstore_download_source_active = bool(
            ui_state.appstore_download_active or process_appstore_active
        )
        if appstore_download_source_active and not appstore_was_active:
            print("App Store: download or update started; enabling the aurora snake.")
        elif not appstore_download_source_active and appstore_was_active:
            print("App Store: last download/update finished; restoring the previous indication.")
        ...
        if arduino_active and not arduino_was_active:
            ...
            print("Arduino IDE: build/upload finished; restoring the previous indication.")
        ...
        appstore_source_active = bool(
            appstore_download_source_active
            or arduino_active
            or manual_appstore_active
        )
        if arduino_active:
            appstore_reason = (
                "Arduino IDE firmware upload"
                if arduino_mode == "upload"
                else "Arduino IDE compilation"
            )
        elif appstore_download_source_active:
            appstore_reason = "App Store download/update"
        else:
            appstore_reason = "manual aurora snake test"
        chatgpt_source_active = (
            codex_session_busy or chatgpt_work_latched or manual_chatgpt_active
        )
        local_copy_source_active = finder_copy_active or manual_copy_active

        chatgpt_foreground = bool(
            (ui_state.safari_foreground and chatgpt_tab_present)
            or (ui_state.openai_app_foreground and openai_app_present)
        )
        finder_foreground = bool(
            ui_state.finder_foreground
            or ui_state.front_bundle == "com.apple.finder"
        )
        ...
        if airdrop_active:
            ...
            if chatgpt_foreground:
                ...
                display_reason = chatgpt_reason or "ChatGPT/Codex — active window selected"
            elif finder_foreground:
                next_display_mode = "copy"
                display_reason = local_copy_reason or "Finder — active window selected"
        ...
        if next_display_mode != displayed_mode:
            ...
            if displayed_mode == "appstore":
                print("Indication priority: aurora snake — %s." % display_reason)
            elif displayed_mode == "chatgpt":
                print("Indication priority: orange snake — %s." % display_reason)
            elif displayed_mode == "copy":
                print("Indication priority: light-green snake — %s." % display_reason)
            elif previous_mode != "none":
                print("Priority indication finished; restoring normal pulsing.")
        elif display_reason and display_reason != last_display_reason:
            print("Current indication source: %s." % display_reason)
        ...
        desired_chatgpt_active = (
            chatgpt_active
            if arduino_main_usb_upload_in_progress
            else displayed_mode == "chatgpt"
        )
        desired_copy_active = (
            copy_active
            if arduino_main_usb_upload_in_progress
            else displayed_mode == "copy"
        )

        command_now = time.monotonic()
        appstore_command_needed = (
            desired_appstore_active != appstore_active
            or (
                desired_appstore_active
                and not usb_transport_paused
                and command_now - last_appstore_keepalive_at >= keepalive_seconds
            )
        )
        if appstore_command_needed and command_now >= next_appstore_command_at:
            ...
            try:
                ...
                print("Failed to change the App Store snake: %s" % error)

        command_now = time.monotonic()
        chatgpt_command_needed = (
            desired_chatgpt_active != chatgpt_active
            or (
                desired_chatgpt_active
                and not usb_transport_paused
                and command_now - last_chatgpt_keepalive_at >= keepalive_seconds
            )
        )
        if chatgpt_command_needed and command_now >= next_chatgpt_command_at:
            ...
            try:
                ...
                if chatgpt_active and not previous_chatgpt:
                    ...
                    print("Orange snake temporarily disabled or the task has finished.")
            except Exception as error:
                next_chatgpt_command_at = time.monotonic() + command_retry_seconds
                print("Failed to change the ChatGPT snake: %s" % error)

        command_now = time.monotonic()
        copy_command_needed = (
            desired_copy_active != copy_active
            or (
                desired_copy_active
                and not usb_transport_paused
                and command_now - last_copy_keepalive_at >= keepalive_seconds
            )
        )
        if copy_command_needed and command_now >= next_copy_command_at:
            ...
            try:
                ...
                if copy_active and not previous_copy:
                    print("Light-green snake enabled: %s." % (display_reason or "file transfer"))
                elif not copy_active and previous_copy:
                    print("Light-green snake disabled; restoring the next active mode.")
            except Exception as error:
                next_copy_command_at = time.monotonic() + command_retry_seconds
                print("Failed to change the file-transfer snake: %s" % error)
        ...
        if last_trash_event_counter is None:
            ...
            last_trash_event_counter = trash_counter
            print("Finder: Trash emptying detected; system red indication has priority.")
        ...
        if pending_trash_flashes > 0 and now >= next_trash_command_at:
            try:
                ...
                print("ESP32: three red flashes completed; restoring the previous indication.")
            except Exception as error:
                next_trash_command_at = time.monotonic() + command_retry_seconds
                print("Failed to send Trash-emptying flashes: %s" % error)
        ...
        if pending_system_blue_events > 0 and now >= next_system_blue_command_at:
            try:
                ...
                print(
                    "ESP32: blue system sequence completed; "
                    "restoring the previous indication."
                )
            except Exception as error:
                next_system_blue_command_at = time.monotonic() + command_retry_seconds
                print("Failed to send blue system indication: %s" % error)

        if desired_copy_active and display_reason and display_reason != last_copy_reason:
            print("Active file indication: %s." % display_reason)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="AppleMAC-LED USB Serial: Finder, Safari, App Store, AirDrop, Trash, ChatGPT/Codex, and lighting effects"
    )
    parser.add_argument(
        "--port",
        default=os.environ.get(SERIAL_ENV_PORT),
        help="ESP USB Serial port, for example /dev/cu.usbmodem1101; auto-detected by default",
    )
    parser.add_argument("--baud", type=int, default=DEFAULT_SERIAL_BAUD)
    parser.add_argument("--timeout", type=float, default=12.0)

    subparsers = parser.add_subparsers(dest="mode")
    subparsers.add_parser("status", help="Show ESP32 state and USB port")
    subparsers.add_parser("ports", help="Show detected Serial ports")
    subparsers.add_parser("reboot", help="Reboot ESP32")
    subparsers.add_parser("snake-on", help="Enable service snake")
    subparsers.add_parser("snake-off", help="Disable service snake")
    subparsers.add_parser("copy-on", help="Enable copy snake")
    subparsers.add_parser("copy-off", help="Disable copy snake")
    subparsers.add_parser("chatgpt-on", help="Enable ChatGPT orange snake")
    subparsers.add_parser("chatgpt-off", help="Disable ChatGPT orange snake")
    subparsers.add_parser("appstore-on", help="Enable App Store aurora snake")
    subparsers.add_parser("appstore-off", help="Disable App Store aurora snake")
    subparsers.add_parser("trash-flash", help="Three red Trash-emptying flashes")
    subparsers.add_parser("system-blue", help="One blue system snake pass")
    subparsers.add_parser("usb-pause", help="Release USB port for flashing/Serial Monitor")
    subparsers.add_parser("usb-resume", help="Return USB port to the background agent and rescan")
    subparsers.add_parser("usb-reconnect", help="Force-close a stale descriptor and rediscover the ESP")

    daemon = subparsers.add_parser("daemon", help="LaunchAgent background mode")
    daemon.add_argument("--interval", type=float, default=15.0)
    daemon.add_argument("--copy-poll-interval", type=float, default=0.10)
    return parser


async def async_main(args: argparse.Namespace) -> int:
    ipc_modes = {
        "status", "reboot", "snake-on", "snake-off",
        "copy-on", "copy-off", "chatgpt-on", "chatgpt-off",
        "appstore-on", "appstore-off", "trash-flash", "system-blue",
        "usb-pause", "usb-resume", "usb-reconnect",
    }
    if args.mode in ipc_modes:
        response = await send_command_to_running_agent(args.mode)
    ...
    if args.mode in {"usb-pause", "usb-resume", "usb-reconnect"}:
        raise SerialAgentError("This command requires the background agent to be running")
    ...
    if args.mode == "daemon":
        return await daemon_loop(
            provisioner,
            health_interval=max(args.interval, 5.0),
            copy_poll_interval=min(max(args.copy_poll_interval, 0.02), 0.25),
        )
...
def main() -> int:
    ...
    try:
        ...
        if "permission" in lowered or "not authorized" in lowered:
            print(
                "Check AppleMACLED Agent permissions under "
                "Accessibility, Screen Recording, and access to the destination folder.",
                file=sys.stderr,
            )
        return 1
