...
if ! "$PY" -c 'import serial, objc, AppKit' >/dev/null 2>&1; then
...
daemon_pid_is_live() {
  ...
  [[ "$command" == *"esp32_usb_serial.py daemon"* \
    || "$command" == *"send_usb_to_esp.command daemon"* ]]
}
...
if [[ "${1:-}" == "daemon" && "${APPLEMACLED_DAEMON_CHILD:-0}" != "1" ]]; then
  ...
  if ! daemon_pid_is_live "$daemon_pid"; then
    ...
    echo "USB daemon supervisor: started detached process $daemon_pid."
  else
